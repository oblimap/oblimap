...
!
! Copyright (C) 2020 Thomas Reerink & Michael Kliphuis.
!
...
MODULE oblimap_mapping_module
 ...
 !TYPE(triplet), PARAMETER :: no_contribution = triplet(-999999, -999999, 1.0E8_dp)
  ...
  TYPE oblimap_ddo_type
    ...
    INTEGER , DIMENSION(:  ), pointer :: row_index_destination     ! The row    index of each mapped destination grid point
    INTEGER , DIMENSION(:  ), pointer :: column_index_destination  ! The column index of each mapped destination grid point
    INTEGER , DIMENSION(:  ), pointer :: total_contributions       ! The number of contributing departure points used to estimate the field value of the each destination grid point
    INTEGER , DIMENSION(:,:), pointer :: row_index                 ! The row    indices of the contributing departure points for each destination grid point
    INTEGER , DIMENSION(:,:), pointer :: column_index              ! The column indices of the contributing departure points for each destination grid point
    ...
    integer , dimension(:  ), pointer :: total_contributions_
    integer , dimension(:,:), pointer :: row_index_
    integer , dimension(:,:), pointer :: column_index_
    real(dp), dimension(:,:), pointer :: distance_
    type(MPI_Win) :: row_index_destination_win
    type(MPI_Win) :: column_index_destination_win
    type(MPI_Win) :: total_contributions_win
    type(MPI_Win) :: row_index_win
    type(MPI_Win) :: column_index_win
    type(MPI_Win) :: distance_win
...
CONTAINS
  ...
  SUBROUTINE oblimap_read_sid_file(sid_filename, ddo)
    ...
    USE oblimap_configuration_module, ONLY : C, PAR
    use mpi_helpers_mod
    IMPLICIT NONE
   ...
   !WRITE(UNIT=*, FMT='(2(A, I12))') ' Number of mapped points is: ', ddo%total_mapped_points, ', maximum amount of contributions for one mapped point = ', ddo%maximum_contributions
    ...
    call alloc_shared( ddo%total_mapped_points &
                     ...
                     , ddo%row_index_destination &
                     , ddo%row_index_destination_ &
                     , ddo%row_index_destination_win &
                     , PAR%shared_comm)

    call alloc_shared( ddo%total_mapped_points &
                     , ddo%total_mapped_points0 &
                     , ddo%total_mapped_points1 &
                     , ddo%column_index_destination &
                     , ddo%column_index_destination_ &
                     , ddo%column_index_destination_win &
                     , PAR%shared_comm)

    call alloc_shared( ddo%total_mapped_points &
                     , ddo%total_mapped_points0 &
                     , ddo%total_mapped_points1 &
                     , ddo%total_contributions &
                     , ddo%total_contributions_ &
                     , ddo%total_contributions_win &
                     , PAR%shared_comm)
    ...
    call alloc_shared( ddo%total_mapped_points &
                     , ddo%total_mapped_points0 &
                     , ddo%total_mapped_points1 &
                     , ddo%maximum_contributions, 1, ddo%maximum_contributions &
                     , ddo%row_index &
                     , ddo%row_index_ &
                     , ddo%row_index_win &
                     , PAR%shared_comm)
    ...
    ! TODO check alloc_shared status
    status=0
    IF(status /= 0) THEN
  ...
  SUBROUTINE oblimap_deallocate_ddo(ddo)
    ...
    call MPI_Win_free(ddo%row_index_destination_win   )
    call MPI_Win_free(ddo%column_index_destination_win)
    call MPI_Win_free(ddo%total_contributions_win     )
    call MPI_Win_free(ddo%row_index_win               )
    call MPI_Win_free(ddo%column_index_win            )
    call MPI_Win_free(ddo%distance_win                )
